...
detection_map = {}
def create_detection(weights, source, labels, detect_id, thres=0.25, view_img=False, detect_region=None, project="../../runs/detect", name="exp", isDetect=True):
...
class detection:
    def __init__(self, weights, source, thres, view_img, project, name, labels, detect_id, detect_region, isDetect = True):
        self.thread = None
    ...
    def start_detect(self, tp, duration):
        ...
        if self.thread is None or not self.thread.is_alive():
            ...
            if self.is_detect and self.weights:
                ...
                kwargs = { "tp": tp,  "nosave": nosave}
                self.thread = threading.Thread(target=self.camera, kwargs=kwargs, daemon=True)
            self.thread.start()
        else:
    ...
    def camera(
        ...
        bs = 1  # batch_size
        if webcam:
            dataset = LoadStreams(self.source, img_size=imgsz, stride=32, auto=True, vid_stride=vid_stride, tp= tp)
            bs = len(dataset)
        elif screenshot:
            dataset = LoadScreenshots(self.source, img_size=imgsz, stride=32, auto=True, vid_stride=vid_stride)
        else:
            dataset = LoadImages(self.source, img_size=imgsz, stride=32, auto=True, vid_stride=vid_stride)
        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            if save_img:
                if dataset.mode == "image":
                    ...
                    if vid_path[0] == None or is_timestamp_more_than_second(int(endTime.timestamp()), int(standardTime.timestamp()), 60*1):  # new video
